...
const AddUpdate = (toggleDrawer, open) => {
    ...
    useEffect(() => {
        if (id) {
            ...
            InterviewRequest.index({application : interview.application.key}).then(response => {
    ...
    const handleInterviews= (interviews) =>{
       
        const resInterviews = {results:[]}
        const filterInterview = interviews && interviews.results.filter(iv => iv.application.id === interview.application.key)
        resInterviews.results = filterInterview
    ...
    return (
        <>
            {   
                interviews && interviews.results.length ? 
                
                <InterviewTable
                    isLoading={isLoading}
                    interviews={handleInterviews(interviews)}
                    page={1}
                    handlePageChange={{}}
