...
class InterviewFilter(filters.FilterSet):
    ...
    company_name = filters.CharFilter(
        field_name="application__company_name", lookup_expr="icontains"
    )
    start_date = filters.DateFilter(
        field_name="ist_scheduled_at__date", lookup_expr="gte"
    )
    end_date = filters.DateFilter(
        field_name="ist_scheduled_at__date", lookup_expr="lte"
    )
...
class InterviewAdd(CustomLoginRequiredMixin, generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        self.request.data["created_by"] = request.login_user.id
...
class InterviewUpdate(
    ...
    def put(self, request, pk, format=None):
        ...
        request.data["updated_by"] = request.login_user
