...
class Student(models.Model):
    ...
    application_questionnaire_status = models.CharField(
        "Application Questionnaire Status",
        choices=APPLICATION_QUESTIONNAIRE_STATUS,
        max_length=50,
        default="not_yet",
    )
    ...
    relocation = models.CharField(
        "Relocation", blank=True, null=True, choices=RELOCATION, max_length=50
    )
    degree = models.CharField(
        "Degree", blank=True, null=True, max_length=50, choices=DEGREE
    )
    partner_application_note = models.CharField(
        "Note", blank=True, null=True, max_length=500
    )
    partner_application_explanation_status = models.CharField(
        "Explanation Status",
        default="to_be_explained",
        blank=False,
        null=False,
        max_length=50,
        choices=EXPLANATION_STATUS,
    )

    revature = models.CharField(
        "Revature", blank=True, null=True, max_length=50, choices=PARTNER_COMPANY
    )

    genspark = models.CharField(
        "Genspark", blank=True, null=True, max_length=50, choices=PARTNER_COMPANY
    )

    smoothstack = models.CharField(
        "SmoothStack", blank=True, null=True, max_length=50, choices=PARTNER_COMPANY
    )
